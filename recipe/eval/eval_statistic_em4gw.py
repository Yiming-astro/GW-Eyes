"""Evaluation statistics for EM4GW injection tests.

Reads the summary_em4gw.tsv and corresponding jsonl files to compute:
...
SUMMARY_TSV_PATH = Path("recipe/eval/eval_data/summary_em4gw.tsv")
EVAL_DATA_DIR = Path("recipe/eval/eval_data/em4gw")
...
class EvalResult:
    em_event: str
    gold_gw_event: str
    predicted_gw_event: Optional[str]
    is_correct: bool
...
def load_summary_tsv(summary_path: Path) -> List[Tuple[str, str]]:
    """Load summary_em4gw.tsv file.

    Returns:
        List of (em_event, gold_gw_event) tuples
    """
    results = []
    ...
    for line in lines[1:]:
        ...
        parts = line.split("\t")
        if len(parts) >= 2:
            em_event = parts[0].strip()
            gold_gw_event = parts[1].strip()
            results.append((em_event, gold_gw_event))
...
def main():
    ...
    print(f"Loaded {len(summary)} events from summary_em4gw.tsv")

    # Evaluate each event
    results: List[EvalResult] = []
    for em_event, gold_gw_event in summary:
        jsonl_path = EVAL_DATA_DIR / f"{em_event}.jsonl"
        predicted, input_tokens, output_tokens = parse_counterpart_from_jsonl(jsonl_path)

        is_correct = predicted == gold_gw_event

        results.append(EvalResult(
            em_event=em_event,
            gold_gw_event=gold_gw_event,
            predicted_gw_event=predicted,
        ...
        print(f"  {status} {em_event}: gold={gold_gw_event}, pred={predicted}")

    # Compute and print statistics
    stats = compute_statistics(results)

    print("\n" + "=" * 50)
    print("Evaluation Statistics (EM4GW)")
    print("=" * 50)
